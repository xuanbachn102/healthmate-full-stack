...
const TopDoctors = () => {
    ...
    return (
        <div className='flex flex-col items-center gap-4 my-16 text-[#262626] dark:text-white md:mx-10'>
            ...
            <p className='sm:w-1/3 text-center text-sm dark:text-gray-200'>{t('topDoctors.subtitle')}</p>
                    ...
                    <div onClick={() => { navigate(`/appointment/${item._id}`); scrollTo(0, 0) }} className='border border-[#C9D8FF] dark:border-gray-700 dark:bg-gray-800 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500' key={index}>
                        ...
                        <div className='p-4'>
                            <div className={`flex items-center gap-2 text-sm text-center ${item.available ? 'text-green-500' : "text-gray-500 dark:text-gray-400"}`}>
                                <p className={`w-2 h-2 rounded-full ${item.available ? 'bg-green-500' : "bg-gray-500"}`}></p><p>{item.available ? t('topDoctors.available') : t('topDoctors.notAvailable')}</p>
                            </div>
            ...
            <button onClick={() => { navigate('/doctors'); scrollTo(0, 0) }} className='bg-[#EAEFFF] dark:bg-gray-700 text-gray-600 dark:text-gray-200 px-12 py-3 rounded-full mt-10 dark:hover:bg-gray-600'>{t('topDoctors.more')}</button>
        </div>
